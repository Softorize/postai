...
import { AUTO_GENERATED_HEADER_KEYS } from './RequestBuilder'
...
export function HeadersEditor({
  ...
  // Check if a header key is auto-generated
  const isAutoGenerated = (key: string) => {
    return AUTO_GENERATED_HEADER_KEYS.includes(key.toLowerCase())
  }
  ...
  const handleKeyDown = (e: React.KeyboardEvent, index: number) => {
    if (e.key === 'Tab' && !e.shiftKey && index === items.length - 1) {
      ...
      if (lastItem.key || lastItem.value) {
        ...
        handleAdd()
        setTimeout(() => {
          const inputs = document.querySelectorAll('[data-header-key]')
  ...
  return (
    <div className="space-y-2">
      ...
      <div className="flex items-center gap-2 text-[11px] text-text-secondary uppercase tracking-wider font-medium">
        ...
        <div className="flex-1">Header value</div>
        <div className="w-8" />
      </div>

      {/* Rows */}
      {items.map((item, index) => {
        const isAuto = isAutoGenerated(item.key)
            ...
            <div className="flex-1 relative">
              <input
                ...
                className={clsx(
                  ...
                  isAuto && item.key && 'pr-8'
                )}
              />
              {/* Auto-generated indicator */}
              {isAuto && item.key && (
                <div className="absolute right-2 top-1/2 -translate-y-1/2" title="Auto-generated header">
                  <Sparkles className="w-3.5 h-3.5 text-primary-400/60" />
                </div>
              )}
            </div>
